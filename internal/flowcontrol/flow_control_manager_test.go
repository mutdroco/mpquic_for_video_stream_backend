...
var _ = Describe("Flow Control Manager", func() {
	...
	BeforeEach(func() {
		...
		fcm = NewFlowControlManager(mockCpm, &congestion.RTTStats{}, make(map[protocol.PathID]time.Duration)).(*flowControlManager)
	})
	...
	Context("receiving data", func() {
		...
		Context("window updates", func() {
			...
			It("gets stream level window updates", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				updates := fcm.GetWindowUpdates(false)
	...
	Context("sending data", func() {
		...
		It("gets bytes sent for all stream", func() {
			fcm.NewStream(1, false)
			fcm.NewStream(3, true)
			fcm.NewStream(5, true)
			err := fcm.AddBytesSent(1, 100)
			...
			err = fcm.AddBytesSent(5, 500)
			Expect(err).ToNot(HaveOccurred())
			bytes1, err := fcm.GetBytesSent(1)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes1).To(Equal(protocol.ByteCount(100)))
			bytes2, err := fcm.GetBytesSent(3)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes2).To(Equal(protocol.ByteCount(200)))
			bytes3, err := fcm.GetBytesSent(5)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes3).To(Equal(protocol.ByteCount(500)))
		})

		It("adds bytes retransmitted for all stream contributing to connection level flow control", func() {
			fcm.NewStream(1, false)
			fcm.NewStream(3, true)
			fcm.NewStream(5, true)
			err := fcm.AddBytesRetrans(1, 100)
			Expect(err).ToNot(HaveOccurred())
			err = fcm.AddBytesRetrans(3, 200)
			Expect(err).ToNot(HaveOccurred())
			err = fcm.AddBytesRetrans(5, 500)
			Expect(err).ToNot(HaveOccurred())
			Expect(fcm.connFlowController.bytesRetrans).To(Equal(protocol.ByteCount(200 + 500)))
		})

		It("gets bytes retransmitted for all stream", func() {
			fcm.NewStream(1, false)
			fcm.NewStream(3, true)
			fcm.NewStream(5, true)
			err := fcm.AddBytesRetrans(1, 100)
			...
			err = fcm.AddBytesRetrans(5, 500)
			Expect(err).ToNot(HaveOccurred())
			bytes1, err := fcm.GetBytesRetrans(1)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes1).To(Equal(protocol.ByteCount(100)))
			bytes2, err := fcm.GetBytesRetrans(3)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes2).To(Equal(protocol.ByteCount(200)))
			bytes3, err := fcm.GetBytesRetrans(5)
			Expect(err).ToNot(HaveOccurred())
			Expect(bytes3).To(Equal(protocol.ByteCount(500)))
		})
